...
export const MOMENT_DATETIME_ADAPTER_OPTIONS = new InjectionToken<
  MomentDatetimeAdapterOptions
...
export class MomentDatetimeAdapter extends DatetimeAdapter<Moment> {
  ...
  constructor(
    @Optional() @Inject(DATETIME_LOCALE) dateLocale: string,
    @Optional()
    @Inject(MOMENT_DATETIME_ADAPTER_OPTIONS)
    private _options?: MomentDatetimeAdapterOptions
  ) {
    super();
